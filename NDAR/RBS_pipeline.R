rbs_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_rbs)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='rbs.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='rbs_dictionary.csv', ...)
    return(c('rbs.csv', 'rbs_dictionary.csv'))
  }
  return(res)
}