harter_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_harter)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='harter.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='harter_dictionary.csv', ...)
    return(c('harter.csv', 'harter_dictionary.csv'))
  }
  return(res)
}