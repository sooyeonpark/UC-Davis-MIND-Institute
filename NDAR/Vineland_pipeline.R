vineland_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_vineland)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='vineland.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='vineland_dictionary.csv', ...)
    return(c('vineland.csv', 'vineland_dictionary.csv'))
  }
  return(res)
}