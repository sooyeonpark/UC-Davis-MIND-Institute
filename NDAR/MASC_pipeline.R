masc_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_masc)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='masc.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='masc_dictionary.csv', ...)
    return(c('masc.csv', 'masc_dictionary.csv'))
  }
  return(res)
}