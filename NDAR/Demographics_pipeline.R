demographics_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_demographics)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='demographics.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='demographics_dictionary.csv', ...)
    return(c('demographics.csv', 'demographics_dictionary.csv'))
  }
  return(res)
}