physical_exam_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_physical_exam)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='physical_exam.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='physical_exam_dictionary.csv', ...)
    return(c('physical_exam.csv', 'physical_exam_dictionary.csv'))
  }
  return(res)
}