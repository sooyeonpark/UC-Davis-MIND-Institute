ados_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_ados)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='ados.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='ados_dictionary.csv', ...)
    return(c('ados.csv', 'ados_dictionary.csv'))
  }
  return(res)
}