cshq_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_cshq)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='cshq.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='cshq_dictionary.csv', ...)
    return(c('cshq.csv', 'cshq_dictionary.csv'))
  }
  return(res)
}