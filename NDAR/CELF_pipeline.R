celf_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_celf)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='celf.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='celf_dictionary.csv', ...)
    return(c('celf.csv', 'celf_dictionary.csv'))
  }
  return(res)
}