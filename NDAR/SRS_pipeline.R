srs_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_srs)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='srs.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='srs_dictionary.csv', ...)
    return(c('srs.csv', 'srs_dictionary.csv'))
  }
  return(res)
}