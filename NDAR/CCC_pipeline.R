ccc_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_ccc)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='ccc.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='ccc_dictionary.csv', ...)
    return(c('ccc.csv', 'ccc_dictionary.csv'))
  }
  return(res)
}