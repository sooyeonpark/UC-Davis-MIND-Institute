das_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_das)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='das.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='das_dictionary.csv', ...)
    return(c('das.csv', 'das_dictionary.csv'))
  }
  return(res)
}