adis_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_adis)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='adis.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='adis_dictionary.csv', ...)
    return(c('adis.csv', 'adis_dictionary.csv'))
  }
  return(res)
}