gort_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_gort)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='gort.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='gort_dictionary.csv', ...)
    return(c('gort.csv', 'gort_dictionary.csv'))
  }
  return(res)
}