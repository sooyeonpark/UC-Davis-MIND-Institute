msel_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_msel)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='msel.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='msel_dictionary.csv', ...)
    return(c('msel.csv', 'msel_dictionary.csv'))
  }
  return(res)
}