ssp_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_ssp)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='ssp.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='ssp_dictionary.csv', ...)
    return(c('ssp.csv', 'ssp_dictionary.csv'))
  }
  return(res)
}