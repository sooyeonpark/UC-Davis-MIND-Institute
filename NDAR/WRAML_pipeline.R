wraml_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_wraml)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='wraml.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='wraml_dictionary.csv', ...)
    return(c('wraml.csv', 'wraml_dictionary.csv'))
  }
  return(res)
}