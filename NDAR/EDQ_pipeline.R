edq_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_edq)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='edq.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='edq_dictionary.csv', ...)
    return(c('edq.csv', 'edq_dictionary.csv'))
  }
  return(res)
}