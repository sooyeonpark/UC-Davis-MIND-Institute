scared_parent_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_scared_parent)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='scared_parent.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='scared_parent_dictionary.csv', ...)
    return(c('scared_parent.csv', 'scared_parent_dictionary.csv'))
  }
  return(res)
}