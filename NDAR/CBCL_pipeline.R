cbcl_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_cbcl)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='cbcl.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='cbcl_dictionary.csv', ...)
    return(c('cbcl.csv', 'cbcl_dictionary.csv'))
  }
  return(res)
}