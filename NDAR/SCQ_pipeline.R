scq_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_scq)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='scq.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='scq_dictionary.csv', ...)
    return(c('scq.csv', 'scq_dictionary.csv'))
  }
  return(res)
}