eowpvt_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_eowpvt)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='eowpvt.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='eowpvt_dictionary.csv', ...)
    return(c('eowpvt.csv', 'eowpvt_dictionary.csv'))
  }
  return(res)
}