nih_toolbox_pipeline <- function(tables, ...){
  ...
  base = dataportal::left_join_tables(base, tables$tbl_nih_toolbox)
  res = dataportal::remap_table(base, 'NDAR')
  
  res$export_write_function = function(table, ...){
    data.table::fwrite(to_local_df(table$data), file='nih_toolbox.csv', ...)
    data.table::fwrite(to_local_df(table$data), file='nih_toolbox_dictionary.csv', ...)
    return(c('nih_toolbox.csv', 'nih_toolbox_dictionary.csv'))
  }
  return(res)
}